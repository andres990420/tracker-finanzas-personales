...
export default function BudgetsTableItems(promps: Propms) {
  ...
  const [colorProgressBar, colorContainer] = ColorSelector(color);
  return (
    <div
      className={`flex border text-center border-gray-300 m-2 justify-between rounded-xl shadow-lg  backdrop-blur-md ${colorContainer}`}
    >
      ...
      <div className="relative w-[55%] px-2 py-2 m-3">
        <div className="relative w-[100%] h-6 bg-gray-200 border border-gray-400 rounded-2xl overflow-hidden">
          <div
            className={`absolute w-[${currentProgress}%] h-[100%]  shadow-2xl  rounded-2xl overflow-hidden  ${colorProgressBar}`}
          ></div>
        </div>
        <p className="text-xl">
